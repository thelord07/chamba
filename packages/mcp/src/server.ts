...
import { registerLoadDesign } from './tools/load-design.js';
import { registerLoadSkills } from './tools/load-skills.js';
import { registerPartition } from './tools/partition.js';
...
import { registerWorktreeEnv } from './tools/worktree-env.js';
import { registerWorktreeStatus } from './tools/worktree-status.js';
...
export function createServer(logger: Logger, services: Services = createNodeServices()): McpServer {
  ...
  registerLoadDesign(server, logger, services);
  registerDesignPrefs(server, logger, services);
  ...
  registerWorktreeStatus(server, logger, services);
  registerConflictPreview(server, logger, services);
  registerPartition(server, logger, services);
  registerWorktreeEnv(server, logger, services);
