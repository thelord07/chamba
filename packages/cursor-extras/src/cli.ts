...
import { CursorInstaller, type CursorInstallResult } from './installer.js';

function buildInstaller(): CursorInstaller {
  const home = homedir();
  return new CursorInstaller({
    ...
    cursorDir: joinPath(home, '.cursor'),
...
/**
 * Install `@chamba/mcp` globally (pinned to this version) so Cursor can launch
...
function summarize(result: CursorInstallResult, global = false): string {
  const launcher = global ? ' (launches the global `chamba-mcp` binary — no npx per spawn)' : '';
  const mcp = result.mcpAdded
    ? `Registered chamba MCP server in ~/.cursor/mcp.json${launcher}`
    : result.mcpAlreadyPresent
      ? 'chamba MCP server already present in ~/.cursor/mcp.json'
  ...
  lines.push('Restart Cursor so it picks up the new commands, agents and MCP server.');
  return lines.join('\n');
}
...
async function main(): Promise<void> {
  ...
  if (command === 'uninstall') {
    ...
    process.stdout.write(
      ...
      } chamba MCP server in ~/.cursor/mcp.json\n`,
    );
    return;
  }
  ...
  process.stderr.write(
    `Unknown command "${command}". Usage: chamba-cursor [install|uninstall] [--force] [--global] [--version]\n`,
  );
  process.exitCode = 1;
}

main().catch((err) => {
  process.stderr.write(`chamba-cursor failed: ${(err as Error).message}\n`);
  process.exit(1);
});
